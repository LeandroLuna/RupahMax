...
import { UserActive, UserActivityData } from '../dataModels/user-activity';

@Injectable()
export class UserActivityService extends UserActivityData {
  private exams = ['Sangue', 'Glicemia', 'Ureia', 'Colesterol'];
  private getRandomExam = () => Math.floor(Math.random() * 4);
  private getRandom = (roundTo: number) => Math.round(Math.random() * roundTo);
  private generateUserActivityRandomData(date) {
    return {
      date,
      pagesVisitCount: this.exams[this.getRandomExam()],
